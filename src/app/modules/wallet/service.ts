...
import {
  initializePayment,
  verifyPayment,
} from "../payment/paystack.service.js";
...
export const listOneWallet = async (userId: string) => {
...
export const verifyTopUp = async (userId: string, reference: string) => {
  ...
  if (transaction.data.status === "success") {
    ...
    if (
      wallet.processedTransactions &&
      wallet.processedTransactions.includes(reference)
    ) {
      console.log("Transaction already processed:", reference);
      throw new BadRequestError(
        `Transaction already processed:", ${reference}`,
        400
      );
    }

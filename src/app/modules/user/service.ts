  ...
  if (referrer) {
    ...
    if (pointSettings) {
      await Point.findOneAndUpdate(
        { userId: referrer._id },
        { $inc: { balance: pointSettings.point } },
...
export const verifyEmail = async (email: string, otp: string) => {
  ...
  if (user.otp !== otp) throw new BadRequestError(`Incorrect Otp`);

  await User.findByIdAndUpdate(
    { _id: user._id },
    { $set: { verifiedEmail: true, otp: null, expired_at: null } },
    { new: true }
  );
...
export const verifyOtpForPasswordReset = async (
  ...
  await User.findOneAndUpdate(
    { _id: otpRecord._id },
    { $set: { otp: null, expired_at: null, password: hashedPassword } }
  );
...
export const code = async (userId: string) => {
  const user = await User.findById(userId).select("-password");
...
export const updateProfile = async (userId: string, image: string) => {
  const user = await User.findById(userId).select("-password");

...
export const trx = async () => {
    ...
    const totalAmount = totalTrxAmount.length > 0 ? totalTrxAmount[0].totalAmount : 0;
    const pendingAmount = totalPendingAmount.length > 0
        ? totalPendingAmount[0].totalPendingAmount
        : 0;
    const failedAmount = totalFailedAmount.length > 0 ? totalFailedAmount[0].totalFailedAmount : 0;
    const successfulAmount = totalFailedAmount.length > 0 ? totalFailedAmount[0].totalFailedAmount : 0;
    return {
        totalAmount,
        pendingAmount,
        failedAmount,
...
export const getTransactionTotals = async () => {
    try {
        ...
        const monthlyTotal = await Transaction.aggregate([
            {
                $match: {
                    data: {
                        $gte: startOfMonth,
                        $lt: new Date(new Date(startOfMonth).setMonth(startOfMonth.getMonth() + 1)),
                    },
                },
            },
            {
        ...
        const weeklyAmount = weeklyTotal.length > 0 ? weeklyTotal[0].totalAmount : 0;
        const monthlyAmount = monthlyTotal.length > 0 ? monthlyTotal[0].totalAmount : 0;
        return {
            dailyAmount,
            weeklyAmount,
            monthlyAmount,

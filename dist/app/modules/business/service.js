...
export const update = async (id, payload) => {
    ...
    if (!existingBiz)
        throw new NotFoundError(`Business not found`);
    const updatedBiz = await Business.findByIdAndUpdate(id, { ...payload }, {
        new: true,
        runValidators: true,
    });
    return {
...
export const updateImage = async (id, images) => {
    const existingBiz = await Business.findById(id);
    if (!existingBiz)
        throw new NotFoundError(`Business not found`);
    const result = await Business.findOneAndUpdate({ _id: id }, { $set: { images: images } }, { new: true });
    return {
        success: true,
        message: `Business Updated`,
        data: result?.toJSON(),

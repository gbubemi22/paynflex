...
import { code, create, getProfile, login, requestPasswordReset, updateProfile, updateUserProfile, verifyEmail, verifyOtpForPasswordReset, } from "./service.js";
...
export const Login = async (req, res, next) => {
    try {
        const deviceName = req.useragent?.platform && req.useragent.platform !== "unknown"
        ...
        res
            .status(StatusCodes.OK)
            .json(await login(phoneNumber, email, password, deviceToken, deviceName, deviceType));
    }
    catch (error) {
        next(error);
    }
...
export const Point = async (req, res, next) => {
    try {
        const userId = req.user.id;
        console.log(userId);
...
export const ConvertPoint = async (req, res, next) => {
    try {
        const userId = req.user.id;

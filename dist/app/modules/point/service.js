...
export const point = async (userId) => {
...
export const convertPoint = async (userId, amount) => {
    const point = await Point.findOne({ userId });
    if (!point)
        throw new NotFoundError(`User does not have any points`);
    const pointSettings = await PointSetting.findOne({});
    if (!pointSettings)
        throw new NotFoundError(`Point settings not configured`);
    if (point.balance < pointSettings.threshold)
        throw new BadRequestError(`User does not have enough points. Minimum withdrawal is ${pointSettings.threshold}`);
    // Calculate the equivalent wallet amount
    const walletAmount = (amount / pointSettings.point) * pointSettings.value;
    // Update the user's wallet balance
    await Wallet.findOneAndUpdate({ userId }, { $inc: { balance: walletAmount } }, { new: true });
    // Deduct the redeemed points from the user's point balance
    await Point.findOneAndUpdate({ userId }, { $inc: { balance: -amount } }, { new: true });
    return {
        success: true,

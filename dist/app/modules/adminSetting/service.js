...
export const create = async (type, rewardPercentage) => {
    const checkType = await AdminSetting.findOne({ type: type });
    if (checkType)
        throw new ConflictError(`Settings already exits`);
...
export const listOneAdminSetting = async (id) => {
    const adminSetting = await AdminSetting.findById(id);
    if (!adminSetting)
        throw new NotFoundError(`Admin setting not found`);
    return {
        success: true,
...
export const update = async (id, rewardPercentage, type) => {
    const adminSetting = await AdminSetting.findById(id);
    if (!adminSetting)
        throw new NotFoundError(`Admin setting not found`);
